...
# def fetch_supplementary_files(dataset_id, base_dir=DATA_DIR):
#     """
#     Download supplementary files (RAW.tar) for a GEO dataset.
    
#     Args:
#         dataset_id (str): GEO dataset ID (e.g., 'GSE68849')
#         base_dir (str): Base directory to store downloaded files
...
#     # Target file path
#     tar_filename = f"{dataset_id}_RAW.tar"
#     tar_filepath = os.path.join(dataset_dir, tar_filename)
    
#     # Check if file already exists
#     if os.path.exists(tar_filepath):
#         print(f"Supplementary file already exists at {tar_filepath}")
#         return tar_filepath
    
#     # Parse the dataset page to get the download link
#     dataset_url = f"https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc={dataset_id}"
#     print(f"Fetching dataset page: {dataset_url}")
    
#     response = requests.get(dataset_url)
#     if response.status_code != 200:
#         raise Exception(f"Failed to access dataset page: HTTP status code {response.status_code}")
    
#     # Parse the HTML to find the download link
#     soup = BeautifulSoup(response.text, 'html.parser')
#     download_link = None
    
#     # Look for links containing "RAW.tar" in href
#     for link in soup.find_all('a', href=True):
#         if f"{dataset_id}_RAW.tar" in link['href']:
#             download_link = "https://www.ncbi.nlm.nih.gov" + link['href']
#             break
    
#     # If parsing failed, use direct link format
#     if not download_link:
#         download_link = f"https://www.ncbi.nlm.nih.gov/geo/download/?acc={dataset_id}&format=file&file={dataset_id}_RAW.tar"
#         print("Using direct download link format")
    
#     print(f"Downloading supplementary file from: {download_link}")
    
#     # Download using wget
#     try:
#         subprocess.run(['wget', '-O', tar_filepath, download_link], check=True)
#         print(f"Successfully downloaded to {tar_filepath}")
#         return tar_filepath
#     except subprocess.CalledProcessError as e:
#         raise Exception(f"Failed to download supplementary file: {e}")
#     except FileNotFoundError:
#         # If wget is not available, fall back to requests
#         print("wget not found, falling back to requests library")
